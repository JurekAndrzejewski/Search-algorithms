...
class SearchBasedPlayer(Player):
    ...
    def bfs(self, visited, queue, obstacles, food, snake):
        parent = {}
        while queue:
            unpassable = False
            currentPos = queue.pop(0) # last elem
            if currentPos == food.position:
                if self.drawVisited:
                    self.visited = visited
                break
            visited.append(currentPos)
            
            head_up = Position(currentPos.x, currentPos.y - 1)
            head_right = Position(currentPos.x + 1, currentPos.y)
            head_down = Position(currentPos.x, currentPos.y + 1)
            head_left = Position(currentPos.x - 1, currentPos.y)
            
            around = [head_up, head_right, head_down, head_left]
            for dir in around:
                if dir not in visited:
                    unpassable = False
                    if dir.check_bounds(GRID_WIDTH, GRID_HEIGHT):
                        unpassable = True
                    else:
                        ...
                        if not unpassable:
                            for ob in obstacles:
                                if dir == ob.position:
                                    ...
                                    break
                    if not unpassable:
                        parent[dir] = currentPos
                        if dir not in visited:
                            visited.append(dir)
                        queue.append(dir)
        if len(queue) == 0 and currentPos is not food.position:
            print("No path found")
            snake.reset()
        else:
            path = self.backtrace(parent, snake.get_head_position(), currentPos)
            self.turn(snake.get_head_position(), path)
            
    def dfs(self, visited, queue, obstacles, food, snake):
        parent = {}
        while queue:
            currentPos = queue.pop() # first elem
            if currentPos == food.position:
                if self.drawVisited:
                    self.visited = visited
                break
    ...
    def djikstra(self, visited, queue, obstacles, food, snake):
        parent = {}
        while queue:
            currentPos = min(queue, key=queue.get)
    ...
    def Astar(self, visited, queue, obstacles, food, snake):
        parent = {}
        while queue:
            currentPos = min(queue, key=queue.get)
            queue.pop(currentPos)
            ...
            for dir in around:
                ...
                if dir not in visited:
                    ...
                    if not unpassable:
                        ...
                        if dir not in visited:
                            visited.append(dir)
                        queue[dir] = (abs(food.position.x - dir.x) + abs(food.position.y - dir.y)) + modifier
        if len(queue) == 0 and currentPos is not food.position:
            print("No path found")
            snake.reset()
        else:
    ...
    def search_path(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        ...
        self.drawVisited = True
        
        self.bfs(visited, queue, obstacles[0], food, snake)
        #self.dfs(visited, queue, obstacles[0], food, snake)
        
        #weightedQueue = {}
        #weightedQueue[snake.get_head_position()] = 0
        #self.djikstra(visited, weightedQueue, obstacles[0], food, snake)
        
        #weightedQueue[snake.get_head_position()] = (abs(food.position.x - snake.get_head_position().x) + abs(food.position.y - snake.get_head_position().y))
        #self.Astar(visited, weightedQueue, obstacles[0], food, snake)
        pass
